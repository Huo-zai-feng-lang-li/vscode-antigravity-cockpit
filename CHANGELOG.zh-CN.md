...
### 修复
- **无感切号稳定性增强**：彻底解决了“等待扩展无感切号响应超时”（12000ms 限制）的问题。通过将耗时的切号后 UI 刷新（积分/配额同步）改为异步后台预取，实现了秒开级的响应反馈，不再受网络抖动干扰。
- **切号逻辑乐观优化**：引入了 30 秒令牌（Token）乐观使用机制，只要令牌未彻底过期即跳过阻塞式的网络刷新，极大提升了连续切换账号时的响应速度。

### 变更
- **超时阶梯对齐**：同步提升了全链路双通道超时阈值（扩展端 30s，宿主 API 20s），确保在极端弱网环境下依然能稳定完成注入。
